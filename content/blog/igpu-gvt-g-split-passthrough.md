...
There are two ways to use iGPU passthrough to VM. The first way is to use the `Full iGPU Passthrough` to VM. The second way is to use the `iGPU GVT-g` technology which allows us to split the iGPU into two parts. We will be covering the `Split iGPU Passthrough`.
...
For better results its recommend to use a [Windows 10/11 Virtual Machine configuration for proxmox](https://pve.proxmox.com/wiki/Windows_10_virtual_machine).
...
Install the latest version of [Intel's Graphics Driver](https://www.intel.com/content/www/us/en/download-center/home.html){target=_blank} or use the [Intel Driver & Support Assistant](https://www.intel.com/content/www/us/en/support-detect.html#tool downloader){target=_blank} installer.

If all went well you should see the following output in `Device Manager` and [GPU-Z](https://www.techpowerup.com/gpuz/){target=_blank}.
...
Debug Messages - Shows Hardware initialization and errors
